    ...
    function User_Login()
    {
        if (in_array($_POST['universe'], $this->config->item('uni_array'))) {
            if (isset($_POST['name']) and isset($_POST['password']))
            {
                ...
                {
                    $this->Error($this->lang->line('error_login_password'));
                }
            }
        } else {
            $this->Error($this->lang->line('error_world'));
        }
    }

    function Check_Double_Login($user, $universe)
    {
        if ($this->config->item('double_login') and ($user->blocked_time < time()) and ($this->session->userdata('id') > 0) and ($user->id != $this->session->userdata('id')) and ($this->session->userdata('universe') == $universe))
        {
            ...
            $user->blocked_why = ($user->double_login < 5) ? $this->lang->line('suspicion_double_login') : $this->lang->line('double_login');

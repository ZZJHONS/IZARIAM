...
class Actions extends Controller
{
    function Actions()
    {
        ...
        {
            $this->Player_Model->Error($this->lang->line('session_expired'));
        }
        else
        {
            // Load the user
    ...
    function abolishColony()
    {
        ...
        {
            $this->Error($this->lang->line('unable_leave_colony'));
        }
    ...
    function abortFleet($mission = 0, $position = 0, $redirect = 'port')
    {
        if (isset($this->Player_Model->missions[$mission]))
        {
            ...
            {
                // colonization
                // If you have not loaded remove the town and return resources
    ...
    function build($position, $id, $redirect = 'city')
    {
            ...
            (SizeOf($this->Player_Model->build_line[$this->Player_Model->town_id]) <= $this->config->item('town_queue_size')) and (SizeOf($this->Player_Model->build_line[$this->Player_Model->town_id]) > 0 and $this->Player_Model->user->premium_account > 0) or SizeOf($this->Player_Model->build_line[$this->Player_Model->town_id]) == 0)
        {
            ...
            if ($wood >= 0 and $wine >= 0 and $marble >= 0 and $crystal >= 0 and $sulfur >= 0)
            {
                ...
                // A string of text just like in the database
                if ($this->Player_Model->now_town->build_line != '')
                {
                ...
                // The building is added to the queue
                // Update the training if there is
            ...
            {
                $this->Error($this->lang->line('construction_enough_resources'));
            }
        }
        else
        {
        ...
        if ($this->Player_Model->now_town->actions > 0)
        {
            if ($id > 0 and $position >= 0)
            {
                ...
                if(isset($_POST['action']))
                {
                    ...
                    if($_POST['action'] == $this->lang->line('move') and isset($this->Player_Model->towns[$_POST['cityId']]) and $this->Player_Model->user->ambrosy >= 200)
                    {
                        $now_position = -1;
                        for ($i = 0; $i <= 15; $i++)
                        {
                        ...
                        if($this->Island_Model->island->$city_text == 0 and $now_position >= 0)
                        {
                            ...
                            $this->db->update($this->session->userdata('universe').'_islands');
                            // Writing a new island in the city
                            ...
                            if(SizeOf($this->Player_Model->towns)-1 < $this->Player_Model->levels[$this->Player_Model->capital_id][10])
                            {
                                ...
                                if(($this->Player_Model->user->transports >= $transporters) and ($wood >= 0) and ($wine >= 0) and ($crystal >= 0) and ($sulfur >= 0) and ($peoples >= 0) and ($gold >= 0) and ($transports >= 0) and ($transporters*$this->config->item('transport_capacity') >= $sendresource + $sendwine + $sendmarble + $sendcrystal + $sendsulfur + 1250 + 40))
                                {
                                    if($this->Island_Model->island->$city_text == 0)
                                    {
                                        ...
                                        // We find a city in the database
    ...
    /**
     * Lowering the level of the building
     * @param <int> $position
     */
    function demolition($position)
    {
        ...
        if ($position > 0 or $this->Player_Model->build_line[$this->Player_Model->town_id][0]['type'] == 1)
        {
            ...
            // If it's zero out Academy of Scientists
            ...
            // If there is a queue, and the building it
            if ($this->Player_Model->now_town->build_line != '' and $this->Player_Model->build_line[$this->Player_Model->town_id][0]['type'] == $type)
            {
                ...
                if ($build_line != '')
                {
                    ...
                    while ($do)
                    {
                        ...
                        {
                            // If there is no decrease of all
                ...
                // Data validation, not to write too much into the database
            ...
            // If the level is not, then we carry the card
        ...
        {
            $this->Error($this->lang->line('impossible_reduce_level'));
        } 
    }
                ...
                if ($chance >= $risk)
                {
                    ...
                    switch($mission)
                    {
                        case 3:
                            ...
                            $text = $this->lang->line('available_this_town').' '.number_format($user->gold).' <img alt="'.$this->lang->line('gold').'" src="'.$this->config->item('style_url').'skin/resources/icon_gold.gif">';
                            break;
                        case 4:
                            $text = '<table cellpadding="0" cellspacing="0" class="reportTable" id="resources">
                                <tr>
                                    <th class="unitname">'.$this->lang->line('resource').'</th>
                                    <th class="count">'.$this->lang->line('quantity').'</th>
                                </tr>
                                <tr>
                                    <td class="unitname"><img src="'.$this->config->item('style_url').'skin/resources/icon_wood.gif" alt="'.$this->lang->line('wood').'" title="'.$this->lang->line('wood').'"></td>
                                    <td class="count">'.number_format($town->wood).'</td>
                                </tr>
                                <tr>
                                    <td class="unitname"><img src="'.$this->config->item('style_url').'skin/resources/icon_wine.gif" alt="'.$this->lang->line('wine').'" title="'.$this->lang->line('wine').'"></td>
                                    <td class="count">'.number_format($town->wine).'</td>
                                </tr>
                                <tr>
                                    <td class="unitname"><img src="'.$this->config->item('style_url').'skin/resources/icon_marble.gif" alt="'.$this->lang->line('marble').'" title="'.$this->lang->line('marble').'"></td>
                                    <td class="count">'.number_format($town->marble).'</td>
                                </tr>
                                <tr>
                                    <td class="unitname"><img src="'.$this->config->item('style_url').'skin/resources/icon_glass.gif" alt="'.$this->lang->line('crystal').'" title="'.$this->lang->line('crystal').'"></td>
                                    <td class="count">'.number_format($town->crystal).'</td>
                                </tr>
                                <tr>
                                    <td class="unitname"><img src="'.$this->config->item('style_url').'skin/resources/icon_sulfur.gif" alt="'.$this->lang->line('sulfur').'" title="'.$this->lang->line('sulfur').'"></td>
                                    <td class="count">'.number_format($town->sulfur).'</td>
                                </tr>
                            </table>';
                            break;
                        case 5:
                            ...
                            $text = '<table cellpadding="0" cellspacing="0" class="reportTable">
                                <tr>
                                    <th class="unitname">'.$this->lang->line('research_areas').'</th>
                                    <th class="count">'.$this->lang->line('current_research').'</th>
                                </tr>
                                <tr>
                                    <td class="unitname">'.$this->lang->line('way1_name').'</td>
                                    <td class="count">'.$ways_names[1].'</td>
                                </tr>
                                <tr>
                                    <td class="unitname">'.$this->lang->line('way2_name').'</td>
                                    <td class="count">'.$ways_names[2].'</td>
                                </tr>
                                <tr>
                                    <td class="unitname">'.$this->lang->line('way3_name').'</td>
                                    <td class="count">'.$ways_names[3].'</td>
                                </tr>
                                <tr>
                                    <td class="unitname">'.$this->lang->line('way4_name').'</td>
                                    <td class="count">'.$ways_names[4].'</td>
                                </tr>
                            </table>';
                            break;
                        case 6:
                            $this->Data_Model->Load_User($town->user);
                            $user = $this->Data_Model->temp_users_db[$town->user];
                            $text = ((time() - $user->last_visit) <= 300) ? $this->lang->line('leader_online') : $this->lang->line('leader_offline');
                            break;
                        case 7:
                            ...
                            $ship_names = '';
                            $ship_counts = '';
                            $text = 'Войска в '.$town->name.'<table cellpadding="0" cellspacing="0" class="reportTable">
                                <tr>
                                    <th class="unitname">'.$this->lang->line('unit').'</th>
                                    <th class="count">'.$this->lang->line('number').'</th>
                                </tr>
                                <tr>';
                            for ($i = 1; $i <= 14; $i++)
                            {
                            ...
                            if ($army_names == '')
                            {
                                $text .= '<td class="unitname">'.$this->lang->line('no_troops').'</td>';
                            ...
                            <tr>
                                <td></td>
                                <td class="report">
                                    '.$this->lang->line('fleets_in').' '.$town->name.'<table cellpadding="0" cellspacing="0" class="reportTable">
                                        <tr>
                                            <th class="unitname">'.$this->lang->line('ships').'</th>
                                            <th class="count">'.$this->lang->line('number').'</th>
                                        </tr>
                                        <tr>';
                            if ($ship_names == '')
                            ...
                            $text .= '</tr>
                                    ...
                                    <tr>
                                        <td></td>
                                        <td class="report">
                                            '.$this->lang->line('blocking_fleet_in').' '.$town->name.'<table cellpadding="0" cellspacing="0" class="reportTable">
                                                <tr>
                                                    <th class="unitname">'.$this->lang->line('ships').'</th>
                                                    <th class="count">'.$this->lang->line('number').'</th>
                                                </tr>
                                                <tr>
                                                    <td class="unitname">'.$this->lang->line('no_troops').'</td>
                                                    <td class="count"></td>
                                                </tr>
                                            </table> ';
                        break;
                        case 8:
                            $text = '<table cellpadding="0" cellspacing="0" class="reportTable">
                                <tr>
                                    <th>'.$this->lang->line('no_fleet_movements').'</th>
                                </tr>
                            </table>';
                        break;
                        case 9:
                            $text = '<table cellpadding="0" cellspacing="0" class="reportTable" width="100%">
                                <tr>
                                    <th class="unitname"><strong>'.$this->lang->line('sender').':</strong></th>
                                    <th class="unitname"><strong>'.$this->lang->line('subject').':</strong></th>
                                    <th class="unitname"><strong>'.$this->lang->line('date').':</strong></th>
                                    <th class="unitname"><strong>'.$this->lang->line('recipient').':</strong></th>
                                </tr>';
                            $text .= '<tr>
                                <td>'.$this->lang->line('no_messages').'</td>
                                <td></td>
                                <td></td>
                                <td></td>
                            </tr> ';
                ...
                else
                {
                    ...
                    if($chance)
                    {
                        ...
                        $spy_message= array(
                            ...
                            'mission' => 0,
                            'date' => time(),
                            'desc' => $this->lang->line('no_get_touch'),
                            'text' => $this->lang->line('no_get_touch_txt').': '.$chance.' %.'
                        );
                        $this->db->insert($this->session->userdata('universe').'_spy_messages', $spy_message);
                    ...
                    else
                    {
                        $spy_message = array(
                            ...
                            'mission' => 0,
                            'date' => time(),
                            'desc' => $this->lang->line('job_canceled'),
                            'text' => $this->lang->line('discovered_bet_returns')
                            );
                        $this->db->insert($this->session->userdata('universe').'_spy_messages', $spy_message);
        ...
        switch($type)
        {
            case 'user':
                if (isset($_POST['name']))
                {
                    ...
                    if ($login != $this->Player_Model->user->login and $login != '')
                    {
                        ...
                        {
                            $this->session->set_flashdata(array('options_error' => $this->lang->line('error')));
                            $this->session->set_flashdata(array('options_error_login' => $this->lang->line('username').' '.$login.' '.$this->lang->line('taken')));
                        }
                    }
                    $position = 'account';
                }
                if (isset($_POST['oldPassword']) and isset($_POST['newPassword']) and isset($_POST['newPasswordConfirm']))
                {
                    ...
                    if ($old != $new and $old != '' and $new != '')
                    {
                        if ($this->Player_Model->user->password == $old)
                        {
                            ...
                            {
                                $this->session->set_flashdata(array('options_error' => $this->lang->line('error')));
                                $this->session->set_flashdata(array('options_error_login' => $this->lang->line('error_password')));
                            }
                        }
                        else
                        {
                            $this->session->set_flashdata(array('options_error' => $this->lang->line('error')));
                            $this->session->set_flashdata(array('options_error_login' => $this->lang->line('error_login_password')));
                        }
                    }
                    $position = 'account';
                }
            ...
            case 'validationEmail':
                ...
                $config['mailtype'] = 'html'; // Type text or html emails
                ...
                if ($this->config->item('game_email'))
                {
                    $message = '
                        ...
                        <body>
                            <p>
                                '.$this->lang->line('register_email_text_1').' '.$this->Player_Model->user->login.', <br>
                                <br>'.$this->lang->line('register_email_text_2').' '.$this->session->userdata('universe').'!<br>
                                ...
                                <br>'.$this->lang->line('username').': '.$this->Player_Model->user->login.'
                                <br>'.$this->lang->line('password').': *****
                                <br>'.$this->lang->line('world').': '.$this->session->userdata('universe').'<br>
                                <br>'.$this->lang->line('register_email_text_5').' ('.$this->config->item('forum_url').').<br>
                                <br>'.$this->lang->line('register_email_text_6').',
                                <br>'.$this->lang->line('register_email_text_7').'
                            </p>
                        </body>
                        </html>';
    ...
    function upgrade($position)
    {
        ...
        {
            $this->Error($this->lang->line('position_of').' '.$position.' '.$this->lang->line('no_build_improve'));
        }
    }

    function winter($special)
    {
        setcookie('Winter', $special);
        redirect($this->config->item('base_url').'game/city/', 'refresh');
    }
    ...
    {
        // Education - hiring workers at a sawmill
